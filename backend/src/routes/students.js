...
// ── GET /api/students/all  (admin/bcba) ───────────────────────────────────────
...
router.get('/', auditLog('VIEW'), async (req, res) => {
  try {
    ...
    if (['admin','bcba'].includes(role)) {
      // Admins and BCBAs always see all students
      ...
      // Teachers only see students if they joined via class code (have assignments)
      const [assignments] = await pool.execute(
        'SELECT COUNT(*) AS cnt FROM student_assignments WHERE user_id = ?', [userId]
      );
      if (assignments[0].cnt > 0) {
        [rows] = await pool.execute(
          `SELECT s.*,
            (SELECT COUNT(*) FROM behavior_logs bl WHERE bl.student_id = s.id AND DATE(bl.start_time) = CURDATE()) AS logs_today,
            (SELECT MAX(bl2.start_time) FROM behavior_logs bl2 WHERE bl2.student_id = s.id) AS last_log_time
        ...
        rows = []; // Unverified — no students until code entered
      }
    } else {
      // Parent / aide — only assigned students
      [rows] = await pool.execute(
        `SELECT s.*,
          (SELECT COUNT(*) FROM behavior_logs bl WHERE bl.student_id = s.id AND DATE(bl.start_time) = CURDATE()) AS logs_today,
          (SELECT MAX(bl2.start_time) FROM behavior_logs bl2 WHERE bl2.student_id = s.id) AS last_log_time
...
router.post('/', requireRole(['admin','bcba','teacher']), async (req, res) => {
  ...
  // Generate a unique 8-char parent code
  const parentCode = Math.random().toString(36).substring(2, 6).toUpperCase() +
                     Math.random().toString(36).substring(2, 6).toUpperCase();

  try {
    const [result] = await pool.execute(
      `INSERT INTO students (organization_id, first_name, last_name, date_of_birth, iep_goals, behavior_plan, sensory_profile, reinforcers, parent_code)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        req.user.organizationId,
...
router.patch('/:id', requireRole(['admin','bcba','teacher']), async (req, res) => {
  ...
  try {
    const [result] = await pool.execute(
      `UPDATE students SET ${fields.join(', ')} WHERE id = ? AND organization_id = ?`, values
    );
