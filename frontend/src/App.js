...
const LoginScreen = ({ onLogin }) => {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      // Auto-login then show join code screen
      const loginData = await apiFetch('/api/auth/login', { method: 'POST', body: JSON.stringify({ email: regEmail, password: regPw }) });
      setFirstName(''); setLastName(''); setRegEmail(''); setRegPw(''); setRegPw2('');
      onLogin(loginData.accessToken, loginData.user, true); // true = show join screen
          ...
          <>
            ...
            {students.length === 0 && !error && (
              <div style={{ textAlign: 'center', padding: '30px 0' }}>
                <div style={{ fontSize: 13, color: th?.textMuted || '#888', marginBottom: 12 }}>No students assigned yet.</div>
                {!['admin','bcba'].includes(user?.role) && (
                  <button onClick={() => navigate('joincode')} style={{ background: '#E1F5EE', border: '1px solid #9FE1CB', borderRadius: 8, padding: '9px 16px', fontSize: 13, color: '#0F6E56', fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>
                    🔑 Enter a class or student code
                  </button>
                )}
              </div>
            )}
...
const JoinCodeScreen = ({ navigate, token, user, th }) => {
  ...
  return (
    <>
      <Topbar th={th} title="Enter a code" subtitle="Link your account to a class or student" />
      <div className="scroll" style={{ background: th?.bg }}>
        <ErrorBanner message={error} />
        <SuccessBanner message={success} />
        <div style={{ background: th?.surface || '#f9f9f9', borderRadius: 10, padding: 14, marginBottom: 20, fontSize: 13, color: th?.textMuted || '#666', lineHeight: 1.6 }}>
          Enter the <strong style={{ color: th?.text || '#1a1a1a' }}>class code</strong> from your admin to see all students, or a <strong style={{ color: th?.text || '#1a1a1a' }}>student code</strong> to link to a specific child as a parent.
        </div>
        <div style={{ marginBottom: 20 }}>
        ...
        <button onClick={() => navigate('home')} style={{ marginTop: 14, background: 'none', border: 'none', fontSize: 12, color: th?.textFaint || '#aaa', cursor: 'pointer', fontFamily: 'inherit', width: '100%', textAlign: 'center' }}>
          Skip for now — I'll enter a code later
        </button>
      </div>
    </>
  );
...
export default function App() {
  ...
  const handleLogin = (accessToken, userData, showJoin = false) => {
    setToken(accessToken);
    setUser(userData);
    setScreen(showJoin ? 'joincode' : 'home');
  };
